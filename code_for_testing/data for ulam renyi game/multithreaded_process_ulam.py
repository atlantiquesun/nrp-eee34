...
def naturalquestion(errors, game_state, constraint, no_classes, trait_breed_matrix ):
...
class UlamRenyi(object):
	def __init__(self, trait_breed_matrix, reliability_of_worker,e):
	...
	def update(self, response):
		...
		if(np.sum(self.sigma_game_state)>1):
			...
			if naturalquestion(self.e, self.game_state, self.constraint,
																 self.no_classes, self.trait_breed_matrix)!=None:
				(self.question_set, self.question) = naturalquestion(self.e, self.game_state, self.constraint,
...
def ulam_sample(i,j,reliability,e):
	np.random.seed()
	game = UlamRenyi(generate_breed_trait_matrix(i, j), reliability,e)
	breed = next(i for i in game.game_state if i != []).pop()
	if breed == game.correct_breed:
		return (1,game.questions_asked)
	else:
		return (0,game.questions_asked)

def ulam_parallel_sample(i,j,reliability,e,iter):
	pool=mp.Pool(5)
	future_res = [pool.apply_async(ulam_sample, args=(i,j,reliability,e)) for _ in range(iter)]
...
if __name__ == '__main__':
	...
	for reliability in reliabilities_to_test:
		...
		i=9
		f = open(('ulam_reliability_{0}_data_traits_{1}_{2}_times.csv'.format(reliability, i, times_to_test)), 'a')
		...
		while(j<=2**i):
			f = open(('ulam_reliability_{0}_data_traits_{1}_{2}_times.csv'.format(reliability, i, times_to_test)), 'a')
			datawriter = csv.writer(f, dialect='excel')
			e=next(e for e in count(1, 1) if (1-reliability)*gamma(set_sigma([list(range(1,j+1))]+[[]]*e))[-1] <= e)
			expected_no=gamma(set_sigma([list(range(1,j+1))]+[[]]*e))[-1]
			results=ulam_parallel_sample(i,j,reliability,e,iter=times_to_test)
