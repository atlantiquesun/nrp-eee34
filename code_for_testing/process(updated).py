...
class UlamRenyi(object):
    ...
    def prepare_arguments(self,constraint,ClassesNum,TraitMatrix):
        ...
        l=list(np.zeros((self.e)))
        zerostates=deepcopy(l)
        for i in range(2):
            for j in range(self.e):
                l[j]+= len(self.game_state[i][j])
        statecapacity=l
    ...
    def generate_question(self,constraint,ClassesNum,TraitMatrix): 
         ...
         '''
        (constraint,classdc,ClassesNum,TraitMatrix,denomdis)=self.prepare_arguments(self,constraint,ClassesNum,TraitMatrix)
        ...
        if i==0:
            ...
            temp1.extend([veclen(temp1[0]),fitness(veclen(temp1[0]),constraint,denomdis),[[k+1,0]],coverage(veclen(temp1[0]))])
            temp2.extend([veclen(temp2[0]),fitness(veclen(temp2[0]),constraint,denomdis),[[k+1,1]],coverage(veclen(temp2[0]))])
            ...
            for k in candidates:
                for q in candidates:
                    if(q!=k): 
                        ...
                        profiles.append([tp,veclen(tp),fitness(veclen(tp),constraint,denomdis),new,coverage(veclen(tp))])#jjjjjjjj
    ...
    def naturalquestion(self,chardic,constraint,ClassesNum,TraitMatrix):
     '''
     ...
     '''
    (l,_ )=self.generate_question(constraint)        
    if l!=[]:
        question=''
...
def generate(ClassesNum,TraitsNum,e):
    ...
    for i in range(ClassesNum):
        ...
        state_sigma[j].append(i+1)
    return (TraitMatrix,question,state_sigma)
...
def singletest(ClassesNum,TraitsNum,e):
    ...
    testcase.naturalquestion(self,chardic=chardic,constraint=constraint,ClassesNum=ClassesNum,TraitMatrix=TraitMatrix)    
    t0=time.clock()
    (a,sol)=solution(ideal,classdc,ClassesNum,TraitMatrix,denomdis)
    t0=time.time()-t0
    print("time take:",t0)
    print("discrepancy:",hammingdistance(ideal,sol))
    print("solution:",a)
    print("coverage vector:",sol)
    print(list2words(a,chardic))
